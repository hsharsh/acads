...
% Since t(i) <= u < t(i+1) sign is being used for calculating N(i,1).
% the code doesn't calculate the last point as because the inequality is <
% and not <=. Since, the last point is same as the last point in the
% definition, we can explicitly define it.
X(end) = p(end,1);
Y(end) = p(end,2);
